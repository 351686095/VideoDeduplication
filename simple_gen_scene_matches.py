...
def main(source, output, metric, metric_threshold, matched_videos):
    ...
    scene_signatures_fp_temp = np.array(glob(os.path.join(source, "**/*.npy"), recursive=True))

    print(f"Number of compatible files found at {source}: {len(scene_signatures_fp_temp)}")
    ...
    video_signatures = []
    scene_index = []
    for sig_path in scene_signatures_fp_temp:
        bname = os.path.basename(sig_path)
        fname = "_".join(bname.split("_")[:-3])
        sigs = np.load(sig_path)
        for i, sig in enumerate(sigs):
            scene_signatures_fp += [sig_path]
            original_filename += [fname]
            video_signatures += [sig]
            scene_index += [i]
    scene_signatures_fp = np.array(scene_signatures_fp)
    original_filename = np.array(original_filename)
    video_signatures = np.array(video_signatures)
    scene_index = np.array(scene_index)

    try:
        matched_videos_np = list(np.load(matched_videos))
    ...
    matched_videos_np = np.unique(matched_videos_np + list(original_filename))
    np.save(matched_videos, matched_videos_np)
